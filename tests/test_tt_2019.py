"""
Truth Table ∩ RBTL — Sep-Oct 2019 Backtest (same window as Win #1/#2)
"""
...
STRATEGIES = [
    ...
    {"label": "RBTL ∩ TT (CY mc≥2)", "grouping": "cluster_year", "mc": 2, "dp": 0, "tt": True},
    ...
    for s in STRATEGIES:
        ...
        try:
            ...
            if found:
                ...
                cell = f"❌ /{n}"
            results[s["label"]].append({"found": found, "rank": rank, "total": n})
...
for s in STRATEGIES:
    ...
    avg_list = sum(r.get("total", 0) for r in active) / len(active) if active else 0
    avg_rank = sum(r["rank"] for r in wins) / len(wins) if wins else 0
