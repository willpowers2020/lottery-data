...
def cmd_promote(args):
    ...
    # Backup
    print(f"\n  📦 Backing up lottery_v2 → {backup_name}...")
...
def cmd_backup(args):
    ...
    backup_count = db[backup_name].count_documents({})
    print(f"\n  ✅ Backup created: {backup_name} ({backup_count:,} records)")
...
def cmd_restore(args):
    ...
    db[COLL_V2].aggregate([{"$match": {}}, {"$out": safety_name}])
    
    # Restore
    print(f"  🔄 Restoring from {latest}...")
    db[COLL_V2].drop()
    db[latest].aggregate([{"$match": {}}, {"$out": COLL_V2}])
